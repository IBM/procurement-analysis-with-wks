...
(function() {
  ...
  syncservice.syncService = function(callback) {
    ...
    if (process.env.APP_SERVICES) {
      const vcapServices = JSON.parse(process.env.APP_SERVICES);
    ...
    if (process.env.VCAP_SERVICES) {
      ...
      const graphServiceName = 'discovery';
    ...
    if (process.env.APP_SERVICES) {
      const vcapServices = JSON.parse(process.env.APP_SERVICES);
      const graphService = 'compose-for-janusgraph';
    ...
    if (process.env.VCAP_SERVICES) {
      const vcapServices = JSON.parse(process.env.VCAP_SERVICES);
      const graphServiceName = 'compose-for-janusgraph';
      if (vcapServices[graphServiceName] && vcapServices[graphServiceName].length > 0) {
        configGraph = vcapServices[graphServiceName][0];
      }
    }
    let graphClient = new JanusGraphClient(
        configGraph.credentials.apiURL,
        configGraph.credentials.username,
        configGraph.credentials.password
    );
